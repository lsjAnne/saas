...
class AuthControllerTest {
    ...
    void shouldLoginWithRegisteredTenantOwner() throws Exception {
        registerTenant("姝ｅ紡绉熸埛", "鑰佹澘", "13800000000");
        ...
        mockMvc.perform(post("/api/auth/login")
                ...
                .andExpect(jsonPath("$.data.user.name").value("鑰佹澘"))
    ...
    @Test
    void shouldLoginWithOwnerMobileWhenUsernameChanged() throws Exception {
        String mobile = "13800000001";
        registerTenant("tenant-b", "owner-b", mobile);
        AuthUser owner = authUserRepository.findByUserId(mobile).orElseThrow();
        ...
        mockMvc.perform(post("/api/auth/login")
                ...
                .andExpect(jsonPath("$.code").value("200"))
                .andExpect(jsonPath("$.data.token").exists())
                .andExpect(jsonPath("$.data.user.id").value(mobile))
                .andExpect(jsonPath("$.data.user.role").value("owner"));
    }
