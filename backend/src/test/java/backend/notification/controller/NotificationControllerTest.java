...
class NotificationControllerTest {
    ...
    void shouldSeedAndUpdateNotificationTemplates() throws Exception {
        ...
        mockMvc.perform(put("/api/notification-templates/{id}", manualTemplateId)
                        ...
                        .content("""
                                {
                                  "templateName": "鎵嬪伐娑堟伅妯℃澘",
                                  "titleTemplate": "鎵嬪伐娑堟伅鏍囬",
                                  "contentTemplate": "鎵嬪伐娑堟伅鍐呭",
                                  "enabled": false
                                }
                                """))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.notificationTemplateId").value(manualTemplateId))
                .andExpect(jsonPath("$.data.templateName").value("鎵嬪伐娑堟伅妯℃澘"))
                .andExpect(jsonPath("$.data.enabled").value(false));
    }

    @Test
    void shouldReadAndUpdateNotificationGatewayConfig() throws Exception {
        NotificationFixture fixture = prepareFixture();

        MvcResult initialConfigResult = mockMvc.perform(get("/api/notification-gateway-config")
                        .header("X-Tenant-Id", fixture.tenantId())
                        .header("X-Operator-Id", "tenant-admin")
                        .header("X-Operator-Type", "tenant-admin"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.requireConfiguredGateway").value(false))
                .andExpect(jsonPath("$.data.channelBindings.length()").value(4))
                .andExpect(jsonPath("$.data.providers.length()").value(4))
                .andReturn();

        JsonNode initialConfig = objectMapper.readTree(initialConfigResult.getResponse().getContentAsString()).path("data");
        assertGatewayBinding(initialConfig.path("channelBindings"), "sms", "aliyun_sms");

        MvcResult updateResult = mockMvc.perform(put("/api/notification-gateway-config")
                        .header("X-Tenant-Id", fixture.tenantId())
                        .header("X-Operator-Id", "tenant-admin")
                        .header("X-Operator-Type", "tenant-admin")
                        .contentType(MediaType.APPLICATION_JSON)
                                  ...
                                  "channelBindings": [
                                    {
                                      ...
                                      "gatewayCode": "site_message_inapp"
                                    },
                                    {
                                      "notifyType": "sms",
                                  ...
                                  "providers": [
                                    {
                                      ...
                                      "endpoint": "internal://notification-center/runtime",
                                      "receiptCallbackPath": "/api/notifications/{id}/delivery-receipts",
                                      "description": "runtime in-app gateway"
                                    },
                                    {
                                      "gatewayCode": "aliyun_sms",
                                      "notifyType": "sms",
                                      ...
                                      "endpoint": "https://runtime-sms.example.com/send",
                                      "receiptCallbackPath": "/api/notifications/{id}/delivery-receipts",
                                      "description": "runtime sms gateway"
                                    }
                                  ]
                                }
                                """))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.requireConfiguredGateway").value(true))
                .andReturn();

        JsonNode updatedConfig = objectMapper.readTree(updateResult.getResponse().getContentAsString()).path("data");
        assertGatewayBinding(updatedConfig.path("channelBindings"), "site_message", "site_message_inapp");
        assertGatewayBinding(updatedConfig.path("channelBindings"), "sms", "aliyun_sms");
        assertGatewayProvider(updatedConfig.path("providers"), "site_message_inapp", "internal://notification-center/runtime", false, "runtime in-app gateway");
        assertGatewayProvider(updatedConfig.path("providers"), "aliyun_sms", "https://runtime-sms.example.com/send", true, "runtime sms gateway");
                        ...
                        .content("""
                                {
                                  ...
                                  "payloadJson": "{\\\"bizType\\\":\\\"runtime-config\\\"}"
                                }
                                """))
                .andExpect(status().isOk())
    ...
    private void assertGatewayBinding(JsonNode channelBindings, String notifyType, String gatewayCode) {
        for (JsonNode channelBinding : channelBindings) {
            if (notifyType.equals(channelBinding.path("notifyType").asText())) {
                if (!gatewayCode.equals(channelBinding.path("gatewayCode").asText())) {
                    throw new IllegalStateException("unexpected gateway binding for " + notifyType);
                }
                return;
            }
        }
        throw new IllegalStateException("gateway binding not found for " + notifyType);
    }

    private void assertGatewayProvider(JsonNode providers,
                                       String gatewayCode,
                                       String endpoint,
                                       boolean mockMode,
                                       String description) {
        for (JsonNode provider : providers) {
            if (gatewayCode.equals(provider.path("gatewayCode").asText())) {
                if (!endpoint.equals(provider.path("endpoint").asText())) {
                    throw new IllegalStateException("unexpected endpoint for provider " + gatewayCode);
                }
                if (mockMode != provider.path("mockMode").asBoolean()) {
                    throw new IllegalStateException("unexpected mockMode for provider " + gatewayCode);
                }
                if (!description.equals(provider.path("description").asText())) {
                    throw new IllegalStateException("unexpected description for provider " + gatewayCode);
                }
                return;
            }
        }
        throw new IllegalStateException("gateway provider not found for " + gatewayCode);
    }
}
