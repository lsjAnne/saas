...
class OpenPlatformControllerTest {
    ...
    private static final String SECOND_OWNER_MOBILE = "13800000001";
                        ...
                        .content("""
                                {
                                  ...
                                  "appType": "erp",
                                  "permissionScope": ["erp.master_data.read", "erp.account_mapping.read"]
                                }
    ...
    @Test
    void shouldRejectDeprecatedOpenPlatformScopesOnCreate() throws Exception {
        OpenPlatformFixture fixture = prepareFixture("open-platform-deprecated-scope-center");
        String ownerToken = login(OWNER_MOBILE, BOOTSTRAP_PASSWORD);
        ...
        mockMvc.perform(post("/api/open/apps")
                        ...
                        .content("""
                                {
                                  "organizationId": "%s",
                                  "appName": "deprecated-scope-app",
                                  "appType": "erp",
                                  "permissionScope": ["finance.read", "order.read"]
                                }
                                """.formatted(fixture.organizationId())))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.code").value("1004"))
                .andExpect(jsonPath("$.message").value("unsupported permissionScope: finance.read, order.read"));
    }

    @Test
    void shouldHideCrossTenantOpenPlatformObjectsFromManagementEndpoints() throws Exception {
        OpenPlatformFixture ownerFixture = prepareFixture("open-platform-owner-center", OWNER_MOBILE);
        OpenPlatformFixture foreignFixture = prepareFixture("open-platform-foreign-center", SECOND_OWNER_MOBILE);
        String ownerToken = login(OWNER_MOBILE, BOOTSTRAP_PASSWORD);
        String foreignToken = login(SECOND_OWNER_MOBILE, BOOTSTRAP_PASSWORD);

        JsonNode foreignApp = createApp(foreignFixture.organizationId(), foreignToken, "foreign-app");
        WebhookFixture foreignWebhook = createWebhook(foreignFixture.organizationId(), foreignToken, "order.foreign");
        ...
        mockMvc.perform(post("/api/open/apps")
                        ...
                        .content("""
                                {
                                  "organizationId": "%s",
                                  "appName": "cross-tenant-app",
                                  "appType": "erp",
                                  "permissionScope": ["erp.master_data.read"]
                                }
                                """.formatted(foreignFixture.organizationId())))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value("1003"))
                .andExpect(jsonPath("$.message").value("object not found"));

        mockMvc.perform(get("/api/open/apps/{id}/credentials", foreignApp.path("appId").asText())
                        .header("Authorization", "Bearer " + ownerToken))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value("1003"))
                .andExpect(jsonPath("$.message").value("object not found"));

        mockMvc.perform(post("/api/open/webhooks/{id}/disable", foreignWebhook.subscriptionId())
                        .header("Authorization", "Bearer " + ownerToken))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value("1003"))
                .andExpect(jsonPath("$.message").value("object not found"));
    ...
    void shouldRefreshCredentialAuthorizeExternalProfileAndRecordLogs() throws Exception {
        ...
        mockMvc.perform(get("/api/open/external/profile")
                ...
                .andExpect(jsonPath("$.data.appType").value("erp"))
                .andExpect(jsonPath("$.data.permissionScope.length()").value(2))
                .andExpect(jsonPath("$.data.permissionScope[0]").value("erp.master_data.read"))
                .andExpect(jsonPath("$.data.permissionScope[1]").value("erp.account_mapping.read"))
                .andExpect(jsonPath("$.data.expiresAt").isString())
                        ...
                        .content("""
                                {
                                  ...
                                  "dictionaryName": "缁撶畻鍑瘉",
                                  "erpCode": "ERP_VOUCHER_SETTLEMENT",
                                  "erpName": "ERP缁撶畻鍑瘉",
                                  "enabled": true,
                                  "remark": "open platform dictionary"
                                }
                                """))
                        */
                        .content("""
                                {
                                  "dictionaryType": "voucher_type",
                                  "dictionaryCode": "settlement_voucher",
                                  "dictionaryName": "Settlement Voucher",
                                  "erpCode": "ERP_VOUCHER_SETTLEMENT",
                                  "erpName": "ERP Settlement Voucher",
                                  "enabled": true,
                                  "remark": "baseline settlement voucher dictionary"
                                }
                                """))
                .andExpect(status().isOk());
                        ...
                        .content("""
                                {
                                  ...
                                  "businessCode": "settlement_voucher",
                                  "subjectCode": "600101",
                                  "subjectName": "Revenue Settlement Subject",
                                  "direction": "credit",
                                  "enabled": true,
                                  "remark": "baseline settlement subject mapping"
                                }
                                """.formatted(storeId)))
                .andExpect(status().isOk());
    ...
    @Test
    void shouldRejectErpExternalAccessWhenGranularScopeMissingAndRecordLogs() throws Exception {
    ...
    @Test
    void shouldRejectExternalErpBaselineWhenTenantIntegrationNotEnabled() throws Exception {
        OpenPlatformFixture fixture = prepareFixture("open-platform-external-disabled-center");
        String ownerToken = login(OWNER_MOBILE, BOOTSTRAP_PASSWORD);
        JsonNode appData = createAppWithScopes(
                fixture.organizationId(),
                ownerToken,
                "external-disabled-app",
                "erp.ofbiz_baseline.read"
        );
        String appId = appData.path("appId").asText();
        confirmSensitivePermission(ownerToken, AuthPermissionCodes.OPENPLATFORM_MANAGE);
        ...
        saasTenantService.updateFeatureToggles(fixture.tenantId(), Map.of(
                "external_integration_erp_required", false
        ));

        mockMvc.perform(get("/api/open/external/erp/ofbiz-baseline")
                        .header("X-Trace-Id", "open-erp-disabled-trace-001")
                        .header("X-Open-App-Key", accessKey)
                        .header("X-Open-App-Secret", secret))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.code").value("1009"))
                .andExpect(jsonPath("$.message").value("tenant external integration erp is not enabled"));
        ...
        mockMvc.perform(get("/api/open/logs")
                ...
                .andExpect(jsonPath("$.data[0].resultStatus").value("created"))
                .andExpect(jsonPath("$.data[1].resultStatus").value("credential_refreshed"))
                .andExpect(jsonPath("$.data[2].resultStatus").value("rejected_integration_disabled"))
                .andExpect(jsonPath("$.data[2].appId").value(appId))
                .andExpect(jsonPath("$.data[2].traceId").value("open-erp-disabled-trace-001"));
    }
    ...
    @Test
    void shouldExposeMissingAcceptanceEvidenceInExternalDeliveryReadinessOverview() throws Exception {
        OpenPlatformFixture fixture = prepareFixture("open-platform-missing-delivery-center");
        String ownerToken = login(OWNER_MOBILE, BOOTSTRAP_PASSWORD);
        JsonNode appData = createAppWithScopes(
                fixture.organizationId(),
                ownerToken,
                "missing-delivery-app",
                "delivery.readiness.read"
        );
        String appId = appData.path("appId").asText();
        confirmSensitivePermission(ownerToken, AuthPermissionCodes.OPENPLATFORM_MANAGE);
        ...
        try {
            mockMvc.perform(get("/api/open/external/delivery/readiness")
                            .header("X-Trace-Id", "open-delivery-missing-trace-001")
                            .header("X-Open-App-Key", accessKey)
                            .header("X-Open-App-Secret", secret))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.data.acceptanceReady").value(false))
                    .andExpect(jsonPath("$.data.acceptance.standardSaas.configured").value(false))
                    .andExpect(jsonPath("$.data.acceptance.standardSaas.verificationFresh").value(false))
                    .andExpect(jsonPath("$.data.acceptance.standardSaas.verificationStatus").value("missing"))
                    .andExpect(jsonPath("$.data.acceptance.standardSaas.verificationAgeDays").value(-1))
                    .andExpect(jsonPath("$.data.acceptance.standardSaas.detail").value(org.hamcrest.Matchers.containsString("verification timestamp missing")))
                    .andExpect(jsonPath("$.data.acceptance.privateDeployment.verificationFresh").value(true))
                    .andExpect(jsonPath("$.data.blockingReasons", org.hamcrest.Matchers.hasItem("dual delivery acceptance is missing standard-saas verification timestamp")));
        } finally {
    ...
    @Test
    void shouldExposeInvalidAcceptanceEvidenceInExternalDeliveryReadinessOverview() throws Exception {
        OpenPlatformFixture fixture = prepareFixture("open-platform-invalid-delivery-center");
        String ownerToken = login(OWNER_MOBILE, BOOTSTRAP_PASSWORD);
        JsonNode appData = createAppWithScopes(
                fixture.organizationId(),
                ownerToken,
                "invalid-delivery-app",
                "delivery.readiness.read"
        );
        String appId = appData.path("appId").asText();
        confirmSensitivePermission(ownerToken, AuthPermissionCodes.OPENPLATFORM_MANAGE);
        ...
        String sourceName = "open-delivery-invalid-evidence-test";
        applyPropertyOverrides(sourceName, Map.of(
        ...
        try {
            mockMvc.perform(get("/api/open/external/delivery/readiness")
                            .header("X-Trace-Id", "open-delivery-invalid-trace-001")
                            .header("X-Open-App-Key", accessKey)
                            .header("X-Open-App-Secret", secret))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.data.acceptanceReady").value(false))
                    .andExpect(jsonPath("$.data.acceptance.standardSaas.verificationFresh").value(false))
                    .andExpect(jsonPath("$.data.acceptance.standardSaas.verificationStatus").value("invalid"))
                    .andExpect(jsonPath("$.data.acceptance.standardSaas.verificationAgeDays").value(-1))
                    .andExpect(jsonPath("$.data.acceptance.privateDeployment.verificationFresh").value(false))
                    .andExpect(jsonPath("$.data.acceptance.privateDeployment.verificationStatus").value("invalid"))
                    .andExpect(jsonPath("$.data.acceptance.privateDeployment.verificationAgeDays").value(-1))
                    .andExpect(jsonPath("$.data.blockingReasons", org.hamcrest.Matchers.hasItem("dual delivery acceptance standard-saas verification timestamp is invalid")))
                    .andExpect(jsonPath("$.data.blockingReasons", org.hamcrest.Matchers.hasItem("dual delivery acceptance private-deployment verification timestamp is invalid")));
        } finally {
            removePropertyOverrides(sourceName);
        }
    }
    ...
    @Test
    void shouldNotMisreportGithubPublishModeWhenGithubOwnerIsMissingInExternalDeliveryReadinessOverview() throws Exception {
        OpenPlatformFixture fixture = prepareFixture("open-platform-missing-github-owner-center");
        String ownerToken = login(OWNER_MOBILE, BOOTSTRAP_PASSWORD);
        JsonNode appData = createAppWithScopes(
                fixture.organizationId(),
                ownerToken,
                "missing-github-owner-app",
                "delivery.readiness.read"
        );
        String appId = appData.path("appId").asText();
        confirmSensitivePermission(ownerToken, AuthPermissionCodes.OPENPLATFORM_MANAGE);
        ...
        String sourceName = "open-delivery-missing-github-owner-test";
        applyPropertyOverrides(sourceName, Map.of(
                "app.delivery.github-owner", ""
        ));
        try {
            mockMvc.perform(get("/api/open/external/delivery/readiness")
                            .header("X-Trace-Id", "open-delivery-missing-github-owner-trace-001")
                            .header("X-Open-App-Key", accessKey)
                            .header("X-Open-App-Secret", secret))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.data.pipelineReady").value(false))
                    .andExpect(jsonPath("$.data.pipeline.githubPublishingControl.configured").value(false))
                    .andExpect(jsonPath("$.data.pipeline.githubPublishingControl.status").value("missing-repository"))
                    .andExpect(jsonPath("$.data.blockingReasons", org.hamcrest.Matchers.hasItem("delivery pipeline is missing github owner")))
                    .andExpect(jsonPath("$.data.blockingReasons", org.hamcrest.Matchers.not(org.hamcrest.Matchers.hasItem("delivery pipeline is missing github publish mode"))))
                    .andExpect(jsonPath("$.data.blockingReasons", org.hamcrest.Matchers.not(org.hamcrest.Matchers.hasItem("delivery pipeline github publish mode must be explicitly configured"))));
        } finally {
            removePropertyOverrides(sourceName);
        }
    }
    ...
    @Test
    void shouldExcludeOptionalExternalIntegrationsFromExternalDeliveryReadinessOverview() throws Exception {
        OpenPlatformFixture fixture = prepareFixture("open-platform-optional-external-center");
        String ownerToken = login(OWNER_MOBILE, BOOTSTRAP_PASSWORD);
        JsonNode appData = createAppWithScopes(
                fixture.organizationId(),
                ownerToken,
                "optional-external-delivery-app",
                "delivery.readiness.read"
        );
        String appId = appData.path("appId").asText();
        confirmSensitivePermission(ownerToken, AuthPermissionCodes.OPENPLATFORM_MANAGE);
        ...
        mockMvc.perform(get("/api/open/external/delivery/readiness")
                        .header("X-Trace-Id", "open-delivery-optional-external-trace-001")
                        .header("X-Open-App-Key", accessKey)
                        .header("X-Open-App-Secret", secret))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.tenantId").value(fixture.tenantId()))
                ...
                .andExpect(jsonPath("$.data.externalIntegrationsReady").value(true))
                .andExpect(jsonPath("$.data.acceptanceReady").value(true))
                .andExpect(jsonPath("$.data.externalIntegrations.configuredCount").value(0))
                .andExpect(jsonPath("$.data.externalIntegrations.reachableCount").value(0))
                .andExpect(jsonPath("$.data.blockingReasons.length()").value(0));
    }
    ...
    void shouldManagePluginCredentialLifecycleAndExposeOpenPlatformOverview() throws Exception {
        ...
        mockMvc.perform(get("/api/open/plugin-governance")
                        .header("Authorization", "Bearer " + ownerToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.totalApps").value(1))
                .andExpect(jsonPath("$.data.activeApps").value(1))
                .andExpect(jsonPath("$.data.riskyApps").value(1))
                .andExpect(jsonPath("$.data.appsMissingCredentials").value(0))
                .andExpect(jsonPath("$.data.organizationsWithoutWebhooks").value(0))
                .andExpect(jsonPath("$.data.entries[0].appId").value(appId))
                .andExpect(jsonPath("$.data.entries[0].credentialStatus").value("revoked"))
                .andExpect(jsonPath("$.data.entries[0].webhookStatus").value("ready"))
                .andExpect(jsonPath("$.data.entries[0].governanceRiskLevel").value("high"))
                .andExpect(jsonPath("$.data.entries[0].issues[0]").value("credential_revoked"));
    }

    private OpenPlatformFixture prepareFixture(String tenantName) throws Exception {
        return prepareFixture(tenantName, OWNER_MOBILE);
    }

    private OpenPlatformFixture prepareFixture(String tenantName, String mobile) throws Exception {
        JsonNode tenantData = registerTenant(tenantName, mobile);
    ...
    private static HttpServer createExternalHttpServer() {
        try {
            ...
            server.createContext("/tax/api", OpenPlatformControllerTest::writeOk);
    ...
    private JsonNode registerTenant(String tenantName) throws Exception {
        return registerTenant(tenantName, OWNER_MOBILE);
    }

    private JsonNode registerTenant(String tenantName, String mobile) throws Exception {
                        ...
                        .content("""
                                {
                                  ...
                                  "aiSummary": "open platform integration test summary"
                                }
                                """.formatted(storeId)))
                        */
                        .content("""
                                {
                                  ...
                                  "sourceType": "1688",
                                  "sourceUrl": "https://source.example.com/p/open-external-3001",
                                  "title": "Portable juicer for external execution",
                                  "category": "kitchen_appliance",
                                  "estimatedProfit": 21.50,
                                  "riskLevel": "low",
                                  "recommendationReason": "external execution baseline for automated publish flow",
                                  "aiSummary": "portable juicer candidate prepared for open platform execution"
                                }
                                """.formatted(storeId)))
                .andExpect(status().isOk())
                .andReturn();
