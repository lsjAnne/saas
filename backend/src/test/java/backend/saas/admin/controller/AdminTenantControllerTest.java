...
class AdminTenantControllerTest {
    ...
    @Test
    void shouldListAdminTenantsWithCreatedAtForOperationsView() throws Exception {
        registerTenant();
    ...
    @Test
    void shouldAllowPlatformAdminToOperateTenantSubscriptionAndSeats() throws Exception {
        String tenantId = registerTenant("tenant-ops-center", "13800000006");

        mockMvc.perform(post("/api/admin/tenants/{id}/subscription/subscribe", tenantId)
                        .header("X-Operator-Id", "platform-ops-1")
                        .header("X-Operator-Type", "platform-ops")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
        ...
        mockMvc.perform(post("/api/admin/tenants/{id}/subscription/upgrade", tenantId)
                        .header("X-Operator-Id", "platform-ops-1")
                        .header("X-Operator-Type", "platform-ops")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
        ...
        mockMvc.perform(post("/api/admin/tenants/{id}/seats/purchase", tenantId)
                        .header("X-Operator-Id", "platform-ops-1")
                        .header("X-Operator-Type", "platform-ops")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
        ...
        mockMvc.perform(post("/api/admin/tenants/{id}/subscription/downgrade", tenantId)
                        .header("X-Operator-Id", "platform-ops-1")
                        .header("X-Operator-Type", "platform-ops")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
        ...
        TenantSubscription subscription = tenantSubscriptionRepository.findByTenantId(tenantId).orElseThrow();
        assertEquals("basic", subscription.planCode());
        assertEquals(5, subscription.seatCount());
    }
    ...
    void shouldExposeReleaseReadinessChecklistWithBlockingReasonsAndEvidence() throws Exception {
        ...
        mockMvc.perform(get("/api/admin/tenants/{id}/release-readiness", tenantId)
                ...
                .andExpect(jsonPath("$.data.checklistItems[4].detail").value(org.hamcrest.Matchers.containsString("probe reachable 6/6")))
    ...
    void shouldExposeStructuredDeliveryReadinessDetails() throws Exception {
        ...
        mockMvc.perform(get("/api/admin/tenants/{id}/delivery-readiness", tenantId)
                ...
                .andExpect(jsonPath("$.data.externalIntegrations.configuredCount").value(6))
                .andExpect(jsonPath("$.data.externalIntegrations.reachableCount").value(6))
                ...
                .andExpect(jsonPath("$.data.externalIntegrations.tax.systemCode").value("tax"))
                .andExpect(jsonPath("$.data.externalIntegrations.tax.provider").value("tax"))
                .andExpect(jsonPath("$.data.externalIntegrations.tax.protocol").value("http"))
                .andExpect(jsonPath("$.data.externalIntegrations.tax.reachable").value(true))
                .andExpect(jsonPath("$.data.externalIntegrations.tax.host").value("127.0.0.1"))
                .andExpect(jsonPath("$.data.externalIntegrations.tax.detail").value("http 200"))
    ...
    @Test
    void shouldBlockDeliveryReadinessWhenAcceptanceVerificationTimestampIsMissing() throws Exception {
        String tenantId = registerTenant("tenant-delivery-missing-evidence", "13800000029");
    ...
    @Test
    void shouldBlockDeliveryReadinessWhenAcceptanceVerificationTimestampIsInvalid() throws Exception {
        String tenantId = registerTenant("tenant-delivery-invalid-evidence", "13800000027");
        String sourceName = "delivery-invalid-evidence-test";
        applyPropertyOverrides(sourceName, Map.of(
        ...
        try {
            mockMvc.perform(get("/api/admin/tenants/{id}/delivery-readiness", tenantId)
                    ...
                    .andExpect(jsonPath("$.data.acceptance.standardSaas.verificationAgeDays").value(-1))
                    .andExpect(jsonPath("$.data.acceptance.standardSaas.detail").value(org.hamcrest.Matchers.containsString("verification timestamp invalid")))
                    .andExpect(jsonPath("$.data.acceptance.privateDeployment.verificationFresh").value(false))
                    ...
                    .andExpect(jsonPath("$.data.acceptance.privateDeployment.detail").value(org.hamcrest.Matchers.containsString("verification timestamp invalid")));
    ...
    @Test
    void shouldNotMisreportGithubPublishModeWhenGithubOwnerIsMissing() throws Exception {
        String tenantId = registerTenant("tenant-delivery-missing-github-owner", "13800000027");
        String sourceName = "delivery-missing-github-owner-test";
        applyPropertyOverrides(sourceName, Map.of(
        ...
        try {
            ...
            mockMvc.perform(get("/api/admin/tenants/{id}/release-readiness", tenantId)
                    ...
                    .andExpect(jsonPath("$.data.checklistItems[?(@.itemCode=='delivery_pipeline_readiness')].detail", org.hamcrest.Matchers.not(org.hamcrest.Matchers.hasItem(org.hamcrest.Matchers.containsString("missing github publish mode")))));
        } finally {
            removePropertyOverrides(sourceName);
        }
    }
    ...
    @Test
    void shouldTreatExternalIntegrationsAsOptionalWhenTenantDoesNotRequireThem() throws Exception {
        ...
        mockMvc.perform(get("/api/admin/tenants/{id}/release-readiness", tenantId)
                        ...
                        .header("X-Operator-Type", "platform-ops"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.checklistItems[?(@.itemCode=='external_integration_readiness')].status",
                        org.hamcrest.Matchers.hasItem("passed")))
                .andExpect(jsonPath("$.data.checklistItems[?(@.itemCode=='external_integration_readiness')].evidenceCount",
                        org.hamcrest.Matchers.hasItem(0)))
                .andExpect(jsonPath("$.data.checklistItems[?(@.itemCode=='external_integration_readiness')].detail",
                        org.hamcrest.Matchers.hasItem(org.hamcrest.Matchers.containsString("current tenant does not require external integrations"))));
                        ...
                        .content("""
                                {
                                  "version": "2026.07",
                                  "title": "闅愮鏀跨瓥",
                                  "content": "updated privacy policy version"
                                }
                                """))
                .andExpect(status().isOk())
                        ...
                        .content("""
                                {
                                  "version": "2026.08",
                                  "title": "闅愮鏀跨瓥",
                                  "content": "new privacy policy version requires reacceptance"
                                }
                                """))
                .andExpect(status().isOk())
    ...
    private String registerTenant() throws Exception {
        return registerTenant("鏄熻抗鐢靛晢", "13800000000");
    }
    ...
    private static HttpServer createExternalHttpServer() {
        try {
            ...
            server.createContext("/tax/api", AdminTenantControllerTest::writeOk);
