package com.dianshang.platform.servicecase.application;
...
public class ServiceCaseService {
    ...
    public CustomerServiceTicket generateReplySuggestion(String tenantId, String ticketId) {
        ...
        String suggestion = """
                您好，关于订单 %s，系统当前状态为 %s。建议先向客户同步处理进展，如涉及退款或补发，将在核实后第一时间处理。
                """.formatted(order.platformOrderId(), order.orderStatus()).trim();
